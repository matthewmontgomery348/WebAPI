...
{
    $startTime = date("Y-m-d H:i:s"); // Get current date and time
    ...
    $stmt = $conn->prepare("INSERT INTO attendance (userName, clock_in) VALUES (?, ?)");
    $stmt->bind_param("ss", $userName, $startTime);

    if ($stmt->execute()) {
        // Return success message as JSON
        //echo json_encode(['success' => true, 'message' => "Clock-in recorded for $userName at $startTime."]);
    } else {
        // Return error as JSON
